...
from services.categories_service import get_category, get_category_posts, list_categories
...
def get_category_posts_route(slug: str, conn=Depends(get_db)):
    category = get_category(conn, slug)
    if not category:
        raise HTTPException(status_code=404, detail="分类不存在")
    posts = get_category_posts(conn, slug)
